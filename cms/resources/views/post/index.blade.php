...
@section('content')
    <div class="card boba-card">
        <div class="card-header boba-card-header text-center clearfix shadow-sm">Post <span class="float-right"><a class="text-dark" href="{{route('post.create')}}"><i class="material-icons">add</i></a></span></div>
        <div class="card-body">
            @if ($posts->count() > 0)
                    ...
                    <tbody class="text-center">
                        @foreach ($posts as $post)
                            <tr>
                                ...
                                <td>
                                    <a class="btn btn-block btn-outline-warning text-dark" href="{{route('category.edit', $post->category->id)}}">
                                        {{$post->category->name}}
                                    </a>
                                </td>
                                <td>
